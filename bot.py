...
def calulator(bot,update,pure_str):
    ...
    if len(list_operators) > 1:
        bot.sendMessage(update.message.chat_id, "операции буду считать последовательно, без учета приоритетов ")
    ...
    try:
        ...
        while num_operands < len(operands):
            op = list_operators[num_operands-1]
            second_operand = float(operands[num_operands])

            result = operation[op](result,second_operand)
            num_operands += 1
        ...
        bot.sendMessage(update.message.chat_id, "ошибка в выражении")
    else:
        

        bot.sendMessage(update.message.chat_id, "ответ "+str(result))
